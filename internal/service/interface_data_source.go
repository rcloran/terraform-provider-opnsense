...
func (d *InterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	resource, err := d.client.Diagnostics().GetInterfaceConfig(ctx)
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to read interfaces, got error: %s", err))
		return
	}

	// Convert OPNsense struct to TF schema
	model, err := convertInterfaceConfigStructToSchema(ctx, &resource, data.Device.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to read interface, got error: %s", resource))
		return
	}
